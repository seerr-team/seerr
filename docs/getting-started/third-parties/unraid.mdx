...
:::caution
The Seerr Default method sets appdata ownership to UID 1000:1000, which conflicts with Unraid's expectation that appdata is owned by `nobody:users` (UID 99, GID 100). This can break SMB share access (users won't be able to edit config files over the network), trigger warnings in the "Fix Common Problems" plugin, and cause issues with backup plugins like CA Appdata Backup or Tailscale.
:::
