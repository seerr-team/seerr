...
Database credentials and the API key can be supplied via Docker secrets (mounted under `/run/secrets`) using the credential names documented in [File-based secrets](/extending-seerr/database-config#file-based-secrets-docker--systemd) and [API Key](/using-seerr/settings/general#api-key).
:::
