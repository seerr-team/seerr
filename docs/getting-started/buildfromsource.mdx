...
3. Create the systemd service file at `/etc/systemd/system/seerr.service`, using either `sudo systemctl edit seerr` or `sudo nano /etc/systemd/system/seerr.service`. Options that may require adjustments are prepended with `#`. Read the comments for more information.
```bash
[Unit]
...
# User=seerr  # The username of the user you're running seerr as. This user must exist.
...
# ReadWritePaths=-/opt/seerr -/etc/seerr # All directories that seerr is allowed to interact with must be listed here. Prepended with a `-` are optional, otherwise it fails if the paths do not exist.
RemoveIPC=yes
...
This unit file expects a dedicated user `seerr` to exist that owns the application directory (and config file).
On Linux, the user can be created using the following command:
`sudo useradd -s /sbin/nologin seerr`
This creates the user `seerr` with no login shell (a non-interactive user used solely for the purpose of running Seerr).
:::
