...
Refer to [Seerr Unraid Documentation](/getting-started/third-parties/unraid) for full installation details. Seerr will automatically migrate your existing Overseerr or Jellyseerr data on first startup. No manual database migration is needed.

**1. Stop the old app**
Stop the existing Overseerr (or Jellyseerr) container from the Unraid **Docker** tab.
**⚠️ Do not remove the container or delete the appdata folder yet ⚠️**

**2. Backup existing data**
Open the Unraid terminal and back up your existing appdata folder:
```bash
...
Because Seerr is now rootless, you must adjust the permissions of your existing appdata folder. Choose your preferred migration path below:

<Tabs groupId="unraid-permissions" queryString>
<TabItem value="seerr-default" label="Seerr Default">
**Seerr Default Migration**
This method aligns with how the Seerr image is officially packaged. Change ownership of the config folder on the host so Seerr runs with its native UID/GID (1000:1000):
```bash
chown -R 1000:1000 /mnt/user/appdata/seerr
```
</TabItem>
<TabItem value="unraid-default" label="Unraid Default">
**Unraid Default Migration**
This method maintains Unraid's default file ownership (`nobody:users`, 99:100). Ensure the config folder ownership matches Unraid's native UID/GID:

:::caution
The Unraid Default method requires running the container with `--user 99:100`, which overrides Seerr's built-in `node` user. Because Seerr is designed to run as a rootless container under its native UID/GID (1000:1000), overriding this may lead to unexpected permission issues or compatibility problems with future Seerr updates.
:::
```bash
...
**4. Install Seerr**
Follow the [Unraid Installation Guide](/getting-started/third-parties/unraid) to add the new Seerr container.

* **Important:** When setting up the container paths, make sure to point the `Host Path` for `/app/config` to your *existing* folder (e.g., `/mnt/user/appdata/seerr`).
* **Note:** If you chose the **Unraid Default** method above, ensure you select the "Unraid Default" tab in the installation guide so you do not forget to add `--user 99:100` to the Extra Parameters!

**5. Start the new Seerr app**
