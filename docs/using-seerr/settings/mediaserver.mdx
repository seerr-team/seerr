...
:::info
Seerr can connect to multiple Plex, Jellyfin, and Emby servers at the same time. You can mix provider types and add more than one server of the same type.

Each configured server has its own internal connection settings, external URL, library selection, and scan state.
:::
...
When adding a new Jellyfin server from **Settings**, you can either provide an API key or sign in with an admin username and password to generate an API key automatically.
:::
...
When adding a new Emby server from **Settings**, you can either provide an API key or sign in with an admin username and password to generate an API key automatically.
:::
