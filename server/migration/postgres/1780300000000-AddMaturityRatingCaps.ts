...
export class AddMaturityRatingCaps1780300000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "user" ADD "maxMovieRating" character varying`
    );
    await queryRunner.query(
      `ALTER TABLE "user" ADD "maxTvRating" character varying`
    );
    await queryRunner.query(
      `ALTER TABLE "user" ADD "ratingBlockUnrated" boolean`
    );
  }
