...
const Login = () => {
  ...
  return (
    <div className="looky-login-page relative flex min-h-screen flex-col overflow-hidden py-14">
      <PageTitle title={intl.formatMessage(messages.signin)} />

      {/* Animated aurora overlay from globals.css */}
      <div className="looky-bg-overlay" />

      <div className="absolute right-4 top-4 z-50">
        <LanguagePicker />
      </div>

      <div className="looky-login-content relative z-40 mt-10 flex flex-col items-center px-4 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="relative h-36 w-full max-w-full">
          <Image src="/logo_stacked.svg" alt="Logo" fill />
        </div>

        <h1 className="mt-3 text-center text-3xl font-extrabold tracking-tight text-cyan-300">
          Lookyoself.org
        </h1>
      ...
      <div className="looky-login-content relative z-50 mt-6 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="looky-glass-card rounded-xl sm:rounded-2xl">
          <Transition
            as="div"
            show={!!error}
            ...
            leaveTo="opacity-0"
          >
            <div className="m-4 rounded-md border border-red-300/30 bg-red-500/20 p-4">
              <div className="flex">
            ...
            {additionalLoginOptions.length > 0 &&
              (loginFormVisible ? (
                <div className="flex items-center py-5">
                  <div className="flex-grow border-t border-slate-500/70" />
                  <span className="mx-2 flex-shrink text-sm text-slate-300">
                    {intl.formatMessage(messages.orsigninwith)}
                  </span>
                  <div className="flex-grow border-t border-slate-500/70" />
                </div>
              ) : (
                <h2 className="mb-6 text-center text-lg font-bold text-slate-100">
